...
async def scrape_listing_pages():
    ...
    while True:
        url = f"https://www.essekna.com/properties?page={page}"
        ...
        cards = soup.find_all("div", class_="col-lg-6 col-md-6 col-sm-6 col-xs-12 item js-marker")
        if not cards:
            print(f"No property cards found on page {page}. Stopping pagination.")
            break
        
        for card in cards:
            # In each card, the property link is inside the <h1 class="title truncate-txt pdb-02">
            title_tag = card.find("h1", class_="title truncate-txt pdb-02")
            if title_tag:
                a_tag = title_tag.find("a")
                if a_tag and a_tag.get("href"):
                    prop_url = a_tag.get("href")
                    if not prop_url.startswith("http"):
                        prop_url = "https://www.essekna.com" + prop_url
...
async def main():
    ...
    with open("immobilier/essekna/data/essekna_properties(51-77).json", "w", encoding="utf-8") as f:
        json.dump(properties, f, ensure_ascii=False, indent=4)
    print("Properties data saved to essekna_properties.json")
