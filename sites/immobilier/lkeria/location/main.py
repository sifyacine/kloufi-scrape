...
async def get_total_pages():
    ...
    url = "https://www.lkeria.com/annonces/immobilier/location-algerie-P1"
    js_commands = [
...
async def scrape_page(page_number):
    global all_results
    url = f"https://www.lkeria.com/annonces/immobilier/location-algerie-P{page_number}"

    # JavaScript commands to accept cookies (if needed)
    js_commands = [
        "await new Promise(resolve => setTimeout(resolve, 5000));",  # Wait 5 seconds for banner
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        if result.success:
            ...
            for annonce in annonces:
                # Titre
                titre_element = annonce.select_one('div.property-title.hidden-xs > a')
                titre = titre_element.text.strip().capitalize() if titre_element else ""

                # URL
                url = titre_element['href'] if titre_element and titre_element.has_attr('href') else ""
                ...
                if image_element:
                    ...
                    print(image_src)
                    image = image_complete([image_src]) if image_src != "//www.lkeria.com/image/vide.jpg" else []
                else:
                    image = []
                ...
                # Date de dépôt
                date_depot_element = annonce.select_one('span.property-label.hidden-xs')
                date_depot = str_todate(date_depot_element.text.strip()) if date_depot_element else datetime.now()

                # Bien category
                category_element = annonce.select_one('div.property-title.hidden-xs')
                bien = ' '.join(
                ...
                # Superficie and Unit
                superficie_text = annonce.select_one('p').text if annonce.select_one('p') else ""
                superficie = str_to_int(re.search(r'(\d+)\s?m2', superficie_text).group(1)) if re.search(r'(\d+)\s?m2', superficie_text) else 0
                ...
                # Description
                description = ' '.join([p.text.strip() for p in annonce.select('p')])
                ...
                # Adresse and Wilaya
                adresse_wilaya = [span.text.strip() for span in annonce.select('span.property-category') if '-' in span.text]
