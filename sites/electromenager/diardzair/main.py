...
async def scrape_listing_pages():
    ...
    js_code = """
            ...
            // Wait for images or product links to load
            await new Promise(resolve => {
                const checkLoad = setInterval(() => {
                    const images = document.querySelectorAll('img');
                    const productLinks = document.querySelectorAll('div.col > a');
                    if (images.length > 10 && productLinks.length > 0) { // Arbitrary threshold for images
