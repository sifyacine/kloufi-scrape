...
async def extract_multimedia_details(url, item):
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        ram = ""
        ram_unit = ""
        ram_match = re.search(r"RAM\s*(\d+)\s*(GO|GB)", result.html, re.IGNORECASE)
        ...
        m_interne = ""
        m_interne_unit = ""
        m_interne_match = re.search(r"DISQUE\s+(\d+)\s*(GO|GB|TB)", result.html, re.IGNORECASE)

        if m_interne_match:
            m_interne = m_interne_match.group(1)
            m_interne_unit = m_interne_match.group(2)

        type_ecran_match = re.search(r"(HD|Super AMOLED|OLED|LCD|TFT)", item["title"] if item["title"] else "")
        processor_cores_match = re.search(r"(Quad-Core|Octa-Core|Hexa-Core|Deca-Core)", result.html)
        processor_hz_match = re.search(r"(\d+\.\d+)GHZ", result.html)
        ...
        item_details = {
            ...
            'm_interne': m_interne if m_interne else "",
            'm_interne_unit': m_interne_unit if m_interne_unit else "",
