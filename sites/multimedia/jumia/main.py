    ...
    async with AsyncWebCrawler(verbose=True) as crawler:
        ...
        while True:
            ...
            if not result.success:
                print(f"Failed to scrape {current_page_url}: {
                      result.error_message}")
                break

            extracted_data = json.loads(result.extracted_content)
            print(f"Extracted {len(extracted_data)
                  } products from page {page_number}")
            ...
            for item in extracted_data:
                if item.get("url"):
                    ...
                    try:
                        ...
                        print(f"Error extracting details for {item['url']}: {str(e)}")
                        continue
                else:
                    continue
            ...
            next_button = soup.find("a", class_="pg", attrs={
                                    "aria-label": "Page suivante"})

            if next_button and "href" in next_button.attrs:
                current_page_url = "https://www.jumia.com.dz" + \
                    next_button["href"]
                page_number += 1
            else:
...
def main():
    # Run the scraper
    urls = ['https://www.jumia.dz/telephone-tablette/', 'https://www.jumia.dz/tv-home-cinema-lecteurs/', 'https://www.jumia.dz/appareils-photo/', 'https://www.jumia.dz/camera-video/','https://www.jumia.dz/electronique-accessoires-fournitures/','https://www.jumia.dz/casques-audios-ecouteurs/','https://www.jumia.dz/surveillance/','https://www.jumia.dz/jeux-et-jouets/']
    for url in urls:
        try:
            asyncio.run(scrape_main_page(url))
        except Exception as e:
